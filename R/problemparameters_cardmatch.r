...
.problemparameters_cardmatch <- function(t_ind, mom_covs, mom_tols, mom_targets, fine_covs) {
  ...
  #! Mom balance
  if (!is.null(mom_covs)) {
    bvec_mom = rep.int(0, length(unique(rows_mom)))
    bvec = c(bvec, bvec_mom)
  }
  #! Fine balance
  if (!is.null(fine_covs)) {
    bvec_fine = rep.int(0, length(unique(rows_fine)))
  ...
  #! Fine balance
  if (!is.null(fine_covs)) {
    sense_fine = rep.int("E", length(unique(rows_fine)))
