meantab <- function(X_mat, t_ind, t_id, c_id, exact = NULL, digits = 2) {
  #t_ind = c(rep(1, length(t_id)), rep(0, length(c_id)))
  #X_mat = X_mat[c(t_id, c_id), ]
  n_vrs <- ncol(X_mat)
  n_obs <- nrow(X_mat)
  output <- matrix(NA_real_, nrow = n_vrs, ncol = 7L,
                   dimnames = list(colnames(X_mat),
                                   c("Mis", "Min", "Max", "Mean T", "Mean C", "Std Dif", "P-val")))

  for (j in seq_len(n_vrs)) {
    ...
    if (n_miss < n_obs) {
      ...
      if (max_val > min_val) {
        ...
        if (is.null(exact)) {
          ...
          p_val <- {
            if (ncol(tab_aux) == 1) 1
            else fisher.test(tab_aux)$p.value
          }
        }
      }
